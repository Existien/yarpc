from .enums_gen.enum_with_arrays_client import EnumWithArraysClient, Color
import asyncio
import random

async def enums_loop(client: EnumWithArraysClient):
    random.seed()
    while True:
        array = [Color(random.randint(0, 3)) for _ in range(0,3)]
        ret = await client.EnumMethod(array)
        print(f"EnumArray: {array}\nReturnValue: {ret}")
...
def get_enums_with_arrays_client():
    client = EnumWithArraysClient()
