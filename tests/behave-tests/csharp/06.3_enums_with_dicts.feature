Feature: EnumsWithDicts interface
    ...
    Background:
        Given a mocked backend service with the following interfaces
            ...
            | EnumsWithDicts    | Bob  |
            | EnumsWithStructs  |      |
        And a running service started with 'csharp_service/run.sh'
        And a mocked python client connecting to the following interfaces
            | interface      | name  |
            | EnumsWithDicts | Alice |

    Scenario: Interface using enums in dictionaries
    ...
    # Scenario: Get all properties
        ...
        Then 'Alice' receives a return value of
            | value                                                                                     |
            | {"EnumProperty":{Color.RED: Color.GREEN,Color.GREEN: Color.RED,Color.BLUE: Color.ORANGE}} |

    # Scenario: Geting and setting read-write properties
        When the 'EnumProperty' property is queried from 'Alice'
        Then 'Alice' receives a return value of
            | value                                                                    |
            | {Color.RED: Color.GREEN,Color.GREEN: Color.RED,Color.BLUE: Color.ORANGE} |
